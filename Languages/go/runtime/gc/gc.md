...
首先，调用 startCycle 开始新周期。按25% CPU利用率计算正式工和小时工数值

随后，结束STW，让MP重回调度循环（schedule）。在这里，检查垃圾回收状态，按需激活worker.G开始垃圾标记工作
...
因gcMarkDone `gcBlackenEnabled = 0`，调度自然不会再去折腾垃圾回收

### 标记

三色标记，需要高性能队列暂存灰色对象。为此，每个P都有本地队列，再以全局队列平衡
...
所有这些（markrootBlock、scanstack）最终将调用scanblock函数
...
灰色对象（存活）需要再 span.gcMarkBits 标记

### 辅助

为避免回收期间，并发执行的用户代码（mutator）大量分配内存，所以引入信用额度进行限制
...
持有数据的是 workbuf 结构，以数组存储多个指针形成数据包。在本地和全局间转移时，也以workbuf为单位，实现批量操作

本地队列 gcWork 持有主从两个 workbuf 缓冲。目的是在本地和全局间平衡，同时避免转移复制

比如主缓冲满，主从对调。而等新换上的主缓冲也满了，就将其整个上交给全局队列，再挂上空白缓冲区。如此，等于平衡一半任务到全局队列
...
如果CAS判断的仅仅是 old 指针地址，而该地址又被意外重用，那就会造成错误结果，这就是所谓ABA问题，利用 地址 + 计数器 生成唯一流水号，实现 Double-CAS，就能避开
