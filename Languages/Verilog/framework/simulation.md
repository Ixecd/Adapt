...
### specify 路径延迟（仿真/时序分析用，RTL 设计少碰）

```
specify
    (a => out) = 9;        // 并行连接（=>）：一对一，a 只影响 out
    (a, b *> out) = 9;     // 全连接（*>）：多对一，a 或 b 变都影响 out，延迟都是 9
endspecify
```

- `=>`（并行）：一个输入只连一个输出（1 对 1）
- `*>`（全连接）：左边每个输入连到右边每个输出（多对多）——`(in1,in2 *> out1,out2)` 四条路径
- 意义：精确到"哪个输入引脚 → 哪个输出引脚"的传播延迟，比四种延迟（门输出状态变化）更细
